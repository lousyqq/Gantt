...
- **環境**：開發＝Sariel\Gantt（另有 Gantt2 測試庫）；遠端正式主機基準＝old.sql+new.sql，增量遷移 10~21。
...
## 最近一次變更（2026-09-16：⚙️ 管理選單被甘特表頭蓋住）

使用者截圖：主管 header「⚙️ 管理 ▾」點開後，「成員管理／瀏覽權限」落在甘特月份列／週次列後面、點不到。
根因與 09-13「顯示 ▾」同一個：選單 `absolute z-[70]` 放在 header 內，header 是 `z-50` flex item＝stacking context，對外只算 50；
甘特 sticky thead 也是 `z-50`、DOM 在後 → thead 蓋掉選單下半。**修法**：選單本體移到 `</header>` 之後當兄弟節點、`fixed z-[70]`＋
`modal-card`，座標點開當下 `getBoundingClientRect()` 存 `adminMenuPos`（與 `displayMenuPos` 同套）。點外關閉／ESC／`isAnyModalOpen` 不變。
實測 1400 寬主管身分：五項 `elementFromPoint` 全命中選單項、「成員管理」開得起面板、點外關閉正常。`npm run build`／check:dark 通過。
CLAUDE.md「顯示 ▾」條目補一段全站規則：**下拉選單本體一律放 header／工具列外面、fixed 定位**。

**同日全站疊層稽核**（使用者問「還有沒有被擋住的視窗」）：程式面盤點所有 fixed／absolute 疊層與其 stacking context；實機
1366／1024 兩種身分逐一開啟 18 個彈窗／面板／選單（含看板開啟時再開彈窗、彈窗內再開 ConfirmModal），對每個可見控制項做
`elementFromPoint` 命中測試 → **全部 0 被擋**。toast 與彈窗儲存鈕也量過不重疊。結論：沒有其他同類問題。

**同日 UI 微調**（使用者部署後貼 1910 寬緊湊模式截圖）：①問「緊湊模式要不要跟年度總覽改成一樣」→ 建議不要（週檢視是操作面、
總覽是任何寬度都保證整年一頁；只在寬螢幕看起來像）。②我提 4 項優化，經討論只做 **🎯 產出鈕固定列尾**（順序改為名稱→▾n→⏰晶片→
＋✎🗑→🎯，實測 69 列右緣同為 479）；不做：條內 ⏰ emoji、未來週次再降階、極短條「功..」（不畫／溢出／單字三案使用者都不要）。
細節與理由已寫進 CLAUDE.md。

**同日：深色模式投影不友善的反應 → 使用者決定不調整、勿再提**。分析結論：投影機黑位抬升（30:1 開燈會議室）會把深色的
三階表面壓成同一灰（格線 vs 面板 1.92→1.61、控制項 vs 面板 2.28→1.51），淺色幾乎不受影響；要救深色得整批提亮、桌機會刺眼。
提了三個小改（開看板時 toast 提醒切淺色、「投影模式」一鍵淺色＋寬鬆、手冊補一句），使用者都不要。處理方式＝口頭提醒投影用淺色。

**同日「企業精簡／降噪」討論的結論（勿再提）**：①header emoji 改 SVG／拿掉 → 不做（沒人抱怨、header 是定案外觀）；
②鍵盤提示收進 tooltip → 不做（窄時本來就自動收、且是全站唯一讓人知道快捷鍵的地方）；
③專案名稱「b.[ … ]」前綴顯示時剝掉 → **不做，使用者：有人需要在 Excel 編輯這段文字，名稱保持原樣**；
④類型晶片收下拉、斜紋改平面 → 不推。剩下 CP 中等的小項（「顯示 69/69 項」只在篩選時顯示、⠿ 把手改 hover、拿掉「系統週數」標籤）
使用者未拍板，未做。概況列標題已從「全隊概況」改「全員概況」（使用者：科技業用「隊」形容組別很突兀）。

**同日：拖曳排序的目標線 off-by-one（使用者問「拖曳排序應該沒問題吧」→ 實測有）**：往下拖時藍線畫在目標上緣、放開卻落在目標
下方（`splice(to)` 在移除後索引已前移一格）；往上拖正確。修法＝線跟著落點走：往下畫目標區塊底緣（子列展開時＝最後一條子列底緣）、
往上畫頂緣，演算法不動（否則最後一個位置放不到）。用合成 DragEvent 實測三種情境一致、資料順序已還原。
⚠ 拖曳只能從 ⠿ 把手開始，拖專案名稱是選字不是拖曳（既有設計，未動）。
⚠ preview_stop 後曾留下孤兒 `Gantt.exe`（PID 19304）鎖住 bin，dotnet run 建置失敗 MSB3027；`Stop-Process -Name Gantt` 即可。

**同日：遠端某站台「使用統計」載入失敗** → 不是程式 bug，是該庫缺遷移。本機比對 `Gantt` vs `Gantt_IMD`：IMD 缺遷移 11
（`AccessRules`＋兩支 SP）與 13（`LoginLogs`＋`usp_LogLogin`），其餘一致。已給 sqlcmd 指令補跑；建議四個群組庫都用
`sys.objects` 差集查一遍。

## 前次變更（2026-09-15：緊湊模式——週欄寬自動填滿＋父列 30→28）

**使用者貼 1926 寬緊湊模式截圖問**「似乎還有空間可利用？有建議再優化嗎」→ 量過後只有一處值得動：`weekW` 緊湊固定 22，
53 週只佔 1166＋凍結欄 490＝1656，右側 ~230px 空白；列高 30→28 是零風險小賺；凍結欄 490、兩列表頭、群組列右側空白**不建議動**
（名稱最長才到 x≈437、表頭各縮 4px 不到一列、群組列摘要條與看板重複）。使用者「照你的建議做」。
**改動**（純前端 `app.jsx`）：①`weekW` 移到 `frozenW` 之後，緊湊＝`clamp(22, floor((availW − 週檢視凍結欄 − 17)/weeksTotal), 32)`，
寬鬆維持 32；新常數 `WEEK_W_RELAXED`／`WEEK_W_COMPACT_MIN`／`SCROLLBAR_W`。②緊湊父列 30→28、條 `bottom` 8→6（條高 18 不變）。
**實測**（玉婷／IMD 庫）：1926→週欄 26px、表 1868、scrollW＝clientW 無橫向捲軸、父列 28、子列 22、條 17（與改前同）；
1600→22（算出 21 夾回）、1366→22 表 1538 照舊橫向捲；1926 開看板→22（表 1656 > 1254 橫向捲）、ESC 關看板→26。
`npm run build`／check:dark 通過。文件：CLAUDE.md 前端慣例新增一條＋列高數字、手冊工具列列的「緊湊模式」括注。
（console 的 `login-log` 500 是既知的 IMD 庫缺 `usp_LogLogin`，與本次無關。）
**同日後續「版面還有哪裡可優化」討論（結論多為不做，勿再提）**：
- 🎯 產出鈕「無產出 hover 才現形」→ **撤回**：使用者要「主管從外層看到哪幾個沒填」，而成果清單 KPI 卡「⚠️ 待補充產出效益 N 案」
  已回答；列上淡點是均勻背景不是雜訊、還標出可點位置。討論過「概況列加 🎯 產出未填 n › 晶片＋面板」，使用者選擇維持原樣。
- 成員群組列 sticky → **撤回**：一群 15~18 案 ≈ 一個畫面內、成員單群組零收益、34px 永久佔用＋第三層 sticky 與白帶；除非主管實際回報迷路。
- 條上名稱黏可視左緣（`overflow: clip`＋sticky）→ 暫不做（只在橫向捲時有感）。
- 概況列＋工具列合併／過去區域灰底／群組列右側摘要 → 不做。
- **已做**：凍結欄表頭 `專案名稱 (Project Name)` → `專案名稱`（全站唯一雙語標籤）。實測 thead 文字已更新。

**同日 bug 修：上下捲動時 sticky 表頭「透」出底下的甘特條**（使用者截圖：年度總覽的月份列／週次列／凍結欄表頭上隱約看得到條與文字）。
排查：thead `sticky z-40`、th 背景不透明、`elementsFromPoint` 是 th 在最上層——是 Chrome **`border-collapse` 表格的繪製問題**，
不是層級問題（`isolation:isolate`／`translateZ(0)` 皆無效，JS 把 table 改 `border-separate` 立即消失）。
**改法**：table `border-collapse` → `border-separate border-spacing-0`；separate 下 `<tr>` 的 border 不畫，故群組列／專案列／子列的
底線與拖曳目標藍線全部移到每個 td（`rowBorder`／`subRowBorder`）；`input.css` 補 `.dark .border-b-blue-100`（排在 `.border-blue-200` 後）。
**實測**：800 寬 1:1 截圖表頭乾淨；表寬 2186＝公式值、凍結格 left 0/28/70/490、列高 40/28/24、子列 26/22/18 全部不變；
半像素列位（24.5/49.5）消失；深色群組底線兩格皆 #244673。拖曳目標藍線未實測（主管操作，class 純搬家）。成果清單那張表未動。
文件：CLAUDE.md 前端慣例新增一條（放在凍結欄遮罩條前）。
**第二輪（使用者再貼三張圖：群組列「玉婷 16 項」整格疊在「No／分類／專案名稱」表頭上，要求表頭兩列絕對固定）**：
根因＝群組列凍結格 `z-40` 與 thead `z-40` 同層級，DOM 較後的 tbody 蓋過 thead。改 thead `z-50`、群組列凍結格 `z-30`
（與專案列凍結格同層）。實測 800 寬 1:1：往下捲 669＋橫捲 120，表頭四個位置 `elementsFromPoint` 都是 th，群組列被裁在週次列下方。
CLAUDE.md 補「thead 50 > 凍結格 30 > 遮罩 20 > 甘特條 10~20」層級表。

## 上一次變更（2026-09-14：版面「減噪」兩項試過即全數退回，程式碼與原樣相同）

**使用者問**「想讓版面更專業簡潔、更符合 UX，還能怎麼調？」→ 答：結構已到位，剩「減噪與層次」，列了 7 項依效益排序；
使用者選做 **1＋2**（純 class 改動）。①月份列 `NAVY` → `slate-200/100` 淺色＋`1月…12月`：使用者貼圖說「特別單調、沒之前好看」
→ 退回。②「📊 團隊總結」`bg-blue-600` → 白框幽靈鈕：使用者「都退回去」→ 退回。
**教訓**：那條深藍是「工具列 vs 資料區」的分界錨點，理論上的「三明治」實際比「整片灰白」好看；兩顆不同色相的實心鈕並排是使用者要的辨識度。
已寫進 CLAUDE.md「勿再提」。app.jsx 只多了兩段說明註解，外觀與改動前完全相同。
**同日另一項（保留）**：使用者問成員的「📋 本週回報中心」會不會太亮太刺眼 → 會，`bg-amber-500` 配白字對比 2.07、是 header 上唯一
還留在 500 級的實心鈕（違反既有「實心配白字用 700 級」規則）。先改成與主管同款 `amber-700/80` → 使用者：深色下與「⏰ 即將到期」撞色、
且主管編輯與成員回報要分色；改 `emerald-700` 翠綠 → 使用者「不好，退回黃色但不要那麼亮」。
**定案＝`bg-amber-600 hover:opacity-90 border-amber-400/80`**，`input.css` 新增 `.dark .bg-amber-600 { #D97706 }` 鎖住深色不壓暗
（否則會被壓成 #B45309 又撞色）。以玉婷身分實測深淺兩色皆 `rgb(217,119,6)`。白字對比 2.86 是使用者明確選擇的例外（已寫進 CLAUDE.md）。
**未做的其餘建議**（使用者日後可挑）：③🎯 產出鈕改「有產出才常駐、無產出 hover／focus-visible 才現形」（69 列淡粉小點成一整條噪音）
④鍵盤提示常駐文字收成 `?` 圖示或進手冊 ⑤三條橫列統一 8px 節奏／兩條白列加分隔 ⑥名稱「b.」前綴與 B 晶片雙重編碼（顯示時去前綴，可選）
⑦「(Project Name)」英文副標可省、週數輸入的虛線提示換淡框。**不建議動**：圖例常駐、計數非按鈕、子條 teal、紅框待回報、群組列。

## 更早一次變更（2026-09-14 深夜：移除概況列四顆狀態晶片的篩選功能，改回純計數）

**使用者兩輪提問**：①「狀態晶片點了就地篩選、⏰ 卻開右側面板，不一致，都改成面板好嗎」→ 答不好，兩者任務不同，只要加分隔線區分。
②貼出「未回報」篩選後的畫面問「這樣呈現好嗎，有點亂；還是我根本不需要這功能，移除更好？」→ **答：移除更好**。篩選只回答「哪幾列」，
回答不了「哪一條該交」（主管視角沒紅框，一列 5 條分不出），而看板已經把「誰沒交／誰沒做」回答得更好（每人回報 x/y＋複製待回報名單
＋點卡片高亮）；同一件事兩條路只多一套要學、多一堆特例。使用者同意移除。
**改動**（純前端）：拿掉 `statusFilter`／`pendingOnly`／`toggleStatusFilter`／`unitMatchesStatus`／`STATUS_FILTER_LABEL`／`StatChip`／
🎉 已全數回報空狀態／activeFilters 的 status 項／isFilteringRows 的 status 條件／登入登出與切成果清單的清除；新 `<StatCount>`
＝色點＋標籤＋數字的靜態標籤（**刻意不像按鈕**，無實心底無 hover），「未回報」不畫色塊（空心紅方塊像核取方塊，使用者回饋醜）、數字 >0 轉 `text-amber-800`。⏰ 即將到期不變。
實測 1600 寬：概況列只剩 ⏰ 一顆 button、四個計數為 span、列高 43 不變。
文件：CLAUDE.md 概況列條改寫（含「勿再加回、若要跳名單做看板預選」）、手冊 §概況列／§11.1、去掉 🎉 相關句。
**若日後要「點數字跳到名單」**：做「點計數 → 開看板並預選該狀態」（看板加狀態篩選），不要回到甘特篩選。

## 更早一次變更（2026-09-14 晚：公司週次規則落地——三處改週日起始、遷移 21、週名帶日期區間）

**使用者確認**：2026-12-31＝W53、2027-01-01＝2027 W01，並問跨年打卡怎麼顯示。**答（已採納）**：照日期拆分、不做合併特例
（跟行事曆／Excel `WEEKNUM(d,1)` 一致）；2027 W01 只有 1/1、1/2（元旦＋週六，0 個上班日），從 1/4 起系統就是 W02，
W01 空著是真實狀態；唯一實務影響＝2026 W53 回報窗口只到週四 12/31，漏交的用既有「歷史補登」在 1 月第一週補，不加新機制。
**改動**：①前端 `companyWeekOf`／`weekDateRange`／`weekRangeLabel`（`getTodayWeek` 改用；`getTodayScheduleYear` 維持日曆年即正確）；
header 週數旁的「2026/09」改成日期區間「9/13–9/19」（月份進 title）、甘特週次表頭 title「W53（12/27–12/31）・…」、
回報中心／補登面板副標前綴日期區間（`PendingPanel` 多收 `scheduleYear`）。②後端 `CompanyWeekOf`，`IsFutureWeek` 不再用 `ISOWeek`。
③遷移 21 `21_company_week_rule.sql`：SP 改寫＋逐年逐週校正（月份 UPDATE、缺週 INSERT、週次編號不動）。
**實測**：Node 跑 15 個邊界日期全對（1/1、1/3、1/4、9/6、9/13、12/27、12/31、2027 1/1、1/2、1/3、12/26、12/31）；本機 Gantt 2026 0 更動、
Gantt_IMD 2026 更正 9 週＋補 W53、2027 兩庫皆成 53 週（6,4,4,4,5,4,4,5,4,5,4,4）；重跑 0 更動；後端 W38 過閘、W39 與 2027 W01 皆 400。
文件：CLAUDE.md（週次規則段改寫、遷移編號 22 起、資料庫節）、DB_table.md 遷移 21、系統架構.md、手冊 §週次。

## 更早一次變更（2026-09-14：年度總覽的子區間列——名稱縮排對齊週檢視＋子條寫名稱）

**使用者回報**（兩張截圖對照）：切年度總覽後子區間名稱「整個變成靠左」（`測試1` 跑到專案名左邊）、子條裡沒有文字；
要求比照週檢視。**原因**：`subIndent` 總覽寫 16，但總覽父列同樣有 `px-2`＋類型晶片＋`mr-2`、專案名從 36px 起；
子條名稱被 `!isOverview` 關掉。**改法**（純前端 `app.jsx` 三處）：`subIndent` 兩種檢視統一 40；子條名稱三種密度都畫、
總覽用 `text-[9px] leading-none px-1`（與父條同級）；總覽子列 16→18、條 `top/bottom 3→2`＝14px 才放得下 9px 字＋3px 色點。
實測 1600 寬：子名稱 x=40 vs 專案名 36、子列高 18、條高 13、條內文字 9px 可見。文件：CLAUDE.md 子區間段兩條。

**同日追加：成果清單欄寬重分**（使用者貼 1926 寬截圖問「有更建議的比例嗎」）。答：留白不是問題，表格內部比例才是——
專案名稱 420 六成空白、NID 128 一行只放 3 顆晶片（7 顆 NID 的列被撐成 3 行）、MP Saving 144 幾乎全是「—」。
改：名稱 300／NID 200＋晶片 `px-1`／MP Saving 112，產出欄 auto 由 ~550 增到 626；`max-w-[1560px]` 維持（產出一行 >90 字會跟丟）。
實測 1926：模擬 7 顆 NID 從 3 行變 2 行（一行 6 顆＝176px 剛好），69 案名稱 62 案單行、6 案兩行。文件：CLAUDE.md 三段檢視條。

## 更早一次變更（2026-09-13 夜：⚙️ 管理選單「歷史補登」項目改名＋名稱統一）
...
## 更早一次變更（2026-09-13 夜：子區間甘特列三項 UX 補強）
...
## 更早一次變更（2026-09-13 夜：A-5 概況列四顆狀態晶片都可點篩選）〔**已於 2026-09-14 依使用者決定移除**，見最近一次變更〕
...
**A 群其餘待做**：~~A-1 週次加日曆日期~~（2026-09-14 已完成，遷移 21）、A-2 甘特條拖曳改期、A-3 排程變更 ↩ 復原（A-5 已於同晚完成，見上）。
...
0. ~~公司週次規則改寫~~ **已完成（2026-09-14，遷移 21）**——剩「遠端跑遷移 21」併入第 1 項。
   ⚠ **遷移 21 務必在 2026-12-27（W53）之前跑到四個站台**：不跑的話後端 `IsFutureWeek` 新版（公司週）與 DB 週表（ISO 版）在 2027 對不上，
   且 2027 只有 52 週、月份表頭全錯。跑之前看 PRINT——某站台 2026 若是 ISO 版會更正 9 週月份＋補 W53，那是修正。
1. **遠端 DB 遷移**：確認遠端是否已依序執行 `10→…→20→21`（未執行則需執行）；Gantt2 測試庫缺 11~21。
   ...
   ⚠ **遷移 19 務必在 2026-12-27（W53，公司週）之前跑到遠端**，否則年底最後一週全員打卡與非專案事項都會 500。
