...
- `POST /api/results-excel` — 下載成果清單 .xlsx（ClosedXML；成果清單頁的「⬇️ 匯出 Excel」按鈕，
  高階主管離線瀏覽專案項目/具體產出/MP Saving；body `{year, projectIds}`，projectIds＝前端目前顯示順序
  （套用篩選與排序），空＝全部）
- 瀏覽權限卡控（遷移 11＋12，2026-07-17）：`GET /api/access-check?empId=&preview=` — 依登入者工號比對
  `[WEB].[dbo].[notes_person]` 名冊（appsettings `Access:PersonView` 可改，即時讀取；遠端為跨 server VIEW，
  本機以 `sim_create_WEB_notes_person.sql` 模擬——**該檔僅開發機用**）與 `AccessRules` 規則。
  **規則＝多欄位組合**（Empno/DeptName/Dept1/Dept2/Dept3 任填 ≥1）：同一條規則內有填的欄位
  **全部符合才通過（AND）**，多條規則之間**任一符合即放行（OR）**；只填工號＝白名單直接放行（不查名冊）。總開關
  `AppSettings.AccessControlEnabled`（預設 false 不卡控）；啟用時查無名冊/查詢失敗/部門不符一律擋下
  （fail-closed），前端整頁顯示 `AccessDeniedScreen`（不給登入）。規則 CRUD：`GET /api/access-rules`、
  `POST /api/access-rule`、`POST /api/access-rule/delete`（SP 內檢查主管，稽核 ACCESSRULE）、
  `POST /api/settings/access-control`（欄位 `enabled`）。前端入口：主管 header「🔐 瀏覽權限」→ AccessPanel
  （開關＋規則管理＋工號測試；**開啟前以主管自身工號測試規則，不通過會擋下開啟**，避免自鎖）。
- 使用統計（遷移 13，2026-07-18）：`POST /api/login-log` — `usp_LogLogin`，每次登入寫一筆 `LoginLogs`
  （前端 handleLogin fire-and-forget；Source：manual=登入畫面點選、auto=重整/重開分頁自動還原；
  60 秒輪詢不計）。`GET /api/login-stats?days=`（7~365，預設 30）回 KPI/manual·auto 拆分/byUser/byDay。
  前端入口：主管 header「📈 使用統計」→ UsageStatsPanel（區間切換 7/30/90 天＋KPI＋每日長條圖＋
  使用者排行）。注意 SQL `SUM(CASE)` 與 `COUNT_BIG` 型別混用，讀取端用 `Convert.ToInt64(GetValue)`。
...
**切週自動置中（2026-07-18）**：header ‹ › 切週與「返回本週」皆透過 `scrollTargetWeek` 機制把甘特圖
平滑捲動置中至該週（勿只 setCurrentWeek 不捲動）。**檢視偏好持久化（2026-07-18）**：localStorage
`gantt_prefs` 記「緊湊模式 compact」與「週檢視/年度總覽 overview」，重整與登入(含自動還原)沿用；
成果清單不記憶；登出不清除。快捷鍵提示常駐於圖例條尾端（⌨ H 回本週・←→ 平移）。
...
**版面收納原則（2026-07-11，2026-07-18 擴充）**：操作元件（工具列/header 按鈕）走小尺寸（11px、py-1），內容區（專案名稱、
甘特、統計數字）維持大字高對比——外殼是配角。**主管 header 依使用頻率收納（2026-07-18）**：只留高頻直達的
「🛠 編輯 W{..} 回報」與「📊 團隊總結」，低頻管理入口（成員管理/瀏覽權限/使用統計/異動紀錄）收進
「⚙️ 管理 ▾」下拉選單（**位於 header 最右側帳號區**，分隔線右邊、使用者名稱左邊——符合「設定/管理在右上角」
的網頁慣例，兩顆高頻鈕相鄰不被打斷；點外/再點/ESC 關閉；選單無輸入內容故點外關閉不違反視窗慣例；
之後新增低頻管理功能一律加進此選單，勿再直接加 header 按鈕）。成員回報入口統一為 header「📋 本週回報中心」
...
- **連線字串即時讀取（2026-07-16）**：`Program.cs` 以 `ConnStr()` local function 每次取用時向
  Configuration 讀取（reloadOnChange）——部署後直接改 `appsettings.json` 的 `Initial Catalog`
  即可切換 DB（如 Gantt→Gantt2 測試庫），存檔數秒內生效，**無須重新發佈或回收集區**。
  勿改回啟動時讀一次的寫法。
- **⚠ 遷移規則（使用者指示，2026-07-12 起生效）**：
  ...
  - 既有新遷移檔：`10_add_manager_weekly_comment.sql`（主管週報回覆，2026-07-13）、
    `11_add_access_control.sql`（瀏覽權限卡控，2026-07-17）、
    `12_access_rules_multi_field.sql`（權限規則改多欄位組合 AND，2026-07-17）、
    `13_add_login_stats.sql`（登入次數統計，2026-07-18）；下一個編號從 14 起。
    另有 `sim_create_WEB_notes_person.sql`＝開發機模擬 [WEB].[dbo].[notes_person] 名冊
    （遠端已有真正的跨 server VIEW，**勿在遠端執行**）。
