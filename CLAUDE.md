...
- `POST /api/project/deliverable` — `usp_UpdateProjectDeliverable`（專案「具體產出項目」＝**專案全部執行完畢後**的
  最終交付成果，專案層級非區間；前端入口為甘特列專案名稱旁的 🎯 圖示 → DeliverableModal；
  僅負責人本人或主管可編輯，SP 內檢查）
...
另有「年度總覽」檢視（isOverview）：table `width:100%`＋名稱單欄 240px，52 週欄自動均分 → 整年一頁無水平捲軸，
唯讀瀏覽（隱藏編輯鈕/拖曳/週數字表頭，條上不顯字，tooltip 與點擊仍可用）。
