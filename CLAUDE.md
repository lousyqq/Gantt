...
- **絕對禁止更改 `old.sql`／`new.sql`**：兩檔為遠端正式環境已執行完畢的架構基準。遠端已有正式資料，
  **嚴禁刪庫／刪表重建**。所有 DB 結構異動一律新增編號遷移檔 `22_xxx.sql`… 往下遞增（10~21 已存在）、
...
- 三段檢視：週檢視（操作）／年度總覽（table width:100% 整年一頁唯讀）／成果清單（高階主管唯讀：無操作欄、
  ...
  ⚠ 成果清單欄寬（2026-09-14 依 1926 寬實測重分）：專案名稱 `w-[300px]`（69 案名稱中位 160、9 成 ≤253，原 420 六成空白）、
  NID `w-[200px]`＋晶片 `px-1`（一行 6 顆；原 128 只塞 3 顆，7 顆 NID 把列撐成 3 行、整張表列高被這欄拖著走）、
  MP Saving `w-28`（80 案只 5 案有值）。外層 `max-w-[1560px]` **維持**：再寬「產出」欄一行超過 90 個中文字會跟丟。
  改欄寬前先量真實資料（名稱長度分佈、NID 顆數），不要憑感覺。
...
- **子區間在甘特圖上＝專案列下方的「縮排子列」，一個子區間一列**（2026-09-11）：重疊的子區間自然成階梯，
  ...
  ⚠ **名稱欄的 `└` 已改成樹狀導引線**（2026-09-13）：每列一個 `└` 在 10 筆子區間時每列都像「最後一筆」，父列捲出畫面後也不知道
    ...
    最後一列底線 `border-slate-300`（其餘 `-200`）標出群組結尾。純 CSS、`aria-hidden`；縮排 `subIndent` **兩種檢視都是 40**
    （2026-09-14 修：總覽原本 16，但總覽父列同樣有 `px-2`＋類型晶片＋`mr-2`、專案名從 36px 起，子區間名稱反而跑到專案名左邊）、
  ...
  ⚠ 拖曳排序中子列**照畫**、掛與父列同一組 `rowDragOver`／`rowDrop`（目標一律 `proj.id`，落在子列＝落在父專案；
    被拖專案的子列一併 `opacity-40`）。曾做成「拖曳中藏子列」——上方有十幾列子區間時整張表
    一開拖就縮短，游標下的目標跳成另一個專案（2026-09-12 修）。
  ⚠ **藍色目標線畫在「實際會落下的位置」**（2026-09-16 實測修）：`handleReorderProjects` 是先移除再 `splice` 到目標索引，
    往下拖落在目標**之後**、往上拖落在目標**之前**；原本一律畫在目標上緣，往下拖時線在 E3 上方、放開卻跑到 E3 下方（off-by-one）。
    **不改演算法**（改成一律「插在目標前」就永遠放不到最後一個位置），改讓線說實話：`dropBelow`＝往下 → 畫目標區塊**底緣**
    （子列展開時是最後一條子列的 `border-b-2 border-b-blue-500`，畫在父列底緣會像插進子列中間）；往上 → 目標頂緣 `border-t-2`。
    實測往下／往上／往下到有子列的專案三種情境，線的位置與落點一致。搜尋 `hay` 含子區間名稱，
...
- **「W.. 概況」的統計跟著 `ownerFilter` 走**（2026-08-10）：標題就寫在被篩選過的表格正上方，
  ...
  標題會同步顯示範圍（`全員概況`／`玉婷概況`），使用者不必用猜的。
  ⚠ **使用者面文案用「全員」不用「全隊」**（2026-09-16 使用者反應：科技業用「隊」形容一個組別很突兀）；與成員下拉的
  「全部成員」同一語彙。「團隊總結」這個功能名維持不變。
  ⚠ 但**不吃搜尋與類型篩選**：那兩個是臨時的「找資料」動作，概況是「這週該做的事完成多少」的固定基準。
- **概況列四個狀態計數是純顯示，不是篩選鈕**（`<StatCount>`：色點＋標籤＋數字，與右側圖例同一種構造；2026-09-14 使用者決定移除篩選）。
  歷史：2026-08-10 把「未回報」做成篩選（看到「未回報 18」後只能在 69 列裡找紅框）、09-13 擴成四顆（旁邊三顆長得一樣使用者會去點）、
  09-14 使用者看到篩選結果（20 列整年甘特）說「有點亂」→ 討論後移除。**移除的理由**：篩選只回答「哪幾列」，回答不了「哪一條該交」
  （主管視角沒有紅框，一列 5 條區間分不出要交哪條）；而「誰還沒交／誰沒做」**團隊看板已回答得更好**（每人回報 x/y＋複製待回報名單
  ＋點卡片高亮那條）。同一件事兩條路只是多一套要學、多一堆特例要維護（篩選態、✕、🎉 空狀態、看板開啟時保留晶片、切成果清單清掉、
  拖曳暫停）。⚠ **樣式刻意不像按鈕**（無圓角實心底、無 hover）：長得像按鈕就會有人去點，09-13 那四顆就是這樣長出來的。
  「未回報」**不畫色塊**（它在甘特上是紅框不是一種顏色，空心紅方塊擺在三個實心旁像沒勾的核取方塊，使用者說醜；紅框圖例右側「❗待回報」已有），警示只走「數字變 `text-amber-800`」一個通道（與看板 0/7 同源）。⚠ **勿再把 `statusFilter` 加回來**；若日後真的需要
  「點數字跳到名單」，做的是「點計數 → 開看板並預選該狀態」（看板加狀態篩選），不是甘特篩選。
...
- **概況列「收無可收」之後的最後一手＝縮間距，不是再刪東西**（`STATS_BAR_MIN_W = 700`，2026-08-10）：
  ...
  ⚠ 極窄時另加 `flex-wrap`：縮完仍有放不下的組合（有子區間時圖例多一格「子區間」，實測 1024 開看板會換成兩行、
  高度 38→68）。**寧可多一行也不要橫向捲軸**——甘特本身就是橫向捲動的，
  同一畫面兩條橫捲軸會分不清在捲哪一個。
...
- **「本週」只存在於今年——時間關係一律走 App 的三個旗標，不要各處自己比 `currentWeek` 與 `todayWeek`**（2026-09-12 修）：
  ...
  ⚠ **週次一律是「公司週」不是 ISO 8601**（2026-09-14 使用者確認後修正）：一週從**週日**開始、W1＝含 1/1 的那一週、跨年**照日期切**
  （12/31 屬舊年度最後一週、1/1 起屬新年度 W1＝Excel `WEEKNUM(d,1)`）。例：2026-12-31(四)＝2026 W53、2027-01-01(五)＝2027 W01
  （只有 1/1、1/2 兩天、0 個上班日，空著是真實狀態）、2027 W02 從 1/3 起；2027 共 53 週（1 月 6 週）。
  規則寫在**三處、要一起改**：前端 `companyWeekOf`（`getTodayWeek` 用它；`getTodayScheduleYear` 就是日曆年）、後端 `CompanyWeekOf`
  （`IsFutureWeek`／`RejectFutureWeek`）、SP `usp_EnsureScheduleYear`（遷移 21，並校正既有年度的月份與缺週）。
  之前三處都是 ISO（週一起始、含 1/4 那週為 W1）→ 每個星期日差一週、跨年整段錯。
  跨年實務：2026 W53 的回報窗口只到週四 12/31（1/1 元旦起系統就是 2027 W01），漏交的人用既有「歷史補登」在 1 月第一週補，不加新機制。
  **週名旁一律帶日期區間** `weekRangeLabel(year, week)`（header 週數旁「09/13–09/19」（MM/DD 零填補，與企業報表及 W01 寫法一致）、甘特週次表頭 `title`、回報中心副標）：
  短週看日期就懂，不必另外解釋；純前端算（`weekDateRange` 夾在 1/1～12/31 內），不進 DB。
...
- **甘特凍結欄的 🎯 產出鈕固定在列尾**（2026-09-16）：順序＝名稱(flex-1) → `▾n` → `⏰ 剩N週` → hover 的 ＋✎🗑 → **🎯 最後**。
  它常駐的理由是「主管從外層一眼看出哪幾案沒填」，要一眼看就得排成一直線；原本排在 ⏰ 晶片前面，帶晶片的列被推左 60px、
  滑過列時再被 ＋✎🗑 推一次。實測 69 列右緣全部 479、hover 時 ＋✎🗑 插在它左邊、列高 40／28 不變。
  ⚠ 同批評估後**不做**的：條內 `⏰` emoji 拿掉（待回報＋到期同時發生時外框是紅的，emoji 是條上唯一的到期訊號）、
  未來週次表頭再降一階（已是 600 vs 700，再降就回到投影不合格的 slate-500）、極短條「功..」改不畫／溢出到空格／單字
  （使用者：不畫像沒填、溢出在長名稱資料上一片亂；省略號本身就是「有名稱但放不下」的訊號，維持現狀）。
...
- 工具列 `flex-nowrap + overflow-x-auto + [&>*]:flex-shrink-0` 不換行；操作元件小尺寸（11px、py-1），內容區大字。
  ...
  ⚠ **header 的「⚙️ 管理」選單同一個坑**（2026-09-16 使用者截圖：「成員管理／瀏覽權限」被甘特月份列／週次列蓋住、點不到）：
  header 是 `z-50` 的 flex item＝自己一個 stacking context，裡面的 `absolute z-[70]` 對外只算 50，而甘特 sticky thead 也是 `z-50`
  且 DOM 在後 → 後者贏。**全站規則：任何下拉選單本體一律放在 header／工具列外面當兄弟節點、`fixed` 定位、座標點開當下從按鈕
  `getBoundingClientRect()` 算**（`adminMenuPos`／`displayMenuPos`），不要再在 header 或工具列裡面用 `absolute` 選單。
...
- 主管 header 只留高頻鈕（🛠 編輯回報／📊 團隊總結），低頻管理入口一律加進右上「⚙️ 管理 ▾」選單。
  ⚠ **「減噪」兩項試過即退回，勿再提**（2026-09-14）：①月份列改 `slate-200/100` 淺色（理由：header 深藍＋月份深藍夾兩條白列
  是「三明治」）→ 實際上半部整片灰白、甘特區沒有錨點，使用者說「特別單調、沒之前好看」；那條深藍是「工具列 vs 資料區」的分界，
  是結構不是噪音。②「📊 團隊總結」`bg-blue-600` 改白框幽靈鈕（理由：header 只留一顆實心主鈕）→ 使用者要求一併退回，
  琥珀＋藍兩顆實心並排是他們要的樣子。現行 header／月份列即定案外觀。
  ⚠ **成員「📋 本週回報中心」＝`bg-amber-600`，深淺兩色鎖同一值 `#D97706`**（2026-09-14 三輪定案）：原本 `amber-500`
  （`#F59E0B`）使用者說「太亮太刺眼」；改主管同款 `amber-700/80` → 深色下 `.dark .bg-amber-500`／700 都壓成 `#B45309`，
  與「⏰ 即將到期」晶片、主管「🛠 編輯回報」撞色；改 `emerald-700` 翠綠 → 使用者「不好，退回黃色但不要那麼亮」。
...
- 開新年度：`EXEC dbo.usp_EnsureScheduleYear <年度>;`（週數以 ScheduleWeeks 筆數為準；遷移 21 起依公司週規則產生，
  週日起始、W1 含 1/1、月份取該週週日）。
